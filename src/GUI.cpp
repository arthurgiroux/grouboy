...
void GUI::displayTileMapView() {
    ...
    for (int x = 0; x < GPU::TILEMAP_WIDTH; x++) {
        for (int y = 0; y < GPU::TILEMAP_HEIGHT; y++) {
            ...
            loadRawRGBInSubTexture(tileMapRenderTexture, x * Tile::TILE_WIDTH, y * Tile::TILE_HEIGHT,
                                   Tile::TILE_WIDTH, Tile::TILE_HEIGHT, tile.toRGB().data());
        }
    }
...
void GUI::displayTileView() {
    ...
    for (int x = 0; x < numberOfTileToDisplayPerLine; x++) {
        for (int y = 0; y < numberOfTileToDisplayPerLine; y++) {
            ...
            loadRawRGBInSubTexture(tileRenderTexture, x * Tile::TILE_WIDTH, y * Tile::TILE_HEIGHT,
                                   Tile::TILE_WIDTH, Tile::TILE_HEIGHT, tile.toRGB().data());
...
void GUI::displayGameView() {
    ImGui::Begin("Game");
    loadRawRGBInTexture(gameRenderTexture, GPU::SCREEN_WIDTH, GPU::SCREEN_HEIGHT, emulator.getGPU().getCurrentFrame().data());
...
bool GUI::initSDL() {
   ...
   glslVersion = "#version 130";
    ...
    mainWindow = SDL_CreateWindow("GameBoy Emulator", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                                  windowSize.x, windowSize.y, window_flags);
